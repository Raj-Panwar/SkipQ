...
public interface OrderRepository extends JpaRepository<Order, Long>, JpaSpecificationExecutor<Order> {
    ...
    @Query(value = """
            ...
            FROM (
                ...
                WHERE college_id = :collegeId
                  ...
                  AND TIMESTAMPDIFF(SECOND, preparing_at, ready_at) BETWEEN 1 AND 3600
                ORDER BY ready_at DESC
                LIMIT 50
            ) recent_orders
            """, nativeQuery = true)
    Double findAveragePreparationSecondsByCollegeId(@Param("collegeId") Long collegeId);
    ...
    @Query("""
                ...
                WHERE o.college.id = :collegeId
                  AND o.status IN ('PLACED', 'PREPARING', 'READY')
            """)
    List<Integer> findActiveTokenNumbersByCollegeId(@Param("collegeId") Long collegeId);
